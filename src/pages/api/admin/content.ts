...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (req.method === "GET") {
    try {
      ...
      return res.status(500).json({ detail: error instanceof Error ? error.message : "Could not load site content" });
    }
  }
  ...
  if (req.method === "PUT") {
    ...
    try {
      ...
      return res.status(500).json({ detail: error instanceof Error ? error.message : "Could not save site content" });
    }
  }
